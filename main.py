...
from model import JointSSPM, SSPM
...
def run(options, train_set, dev_set, my_logger, tb_writer, mode="train"):
    ...
    for epoch in tqdm(range(options["epochs"]), desc="Training ..."):
        ...
        epoch_fn = 0
        epoch_tp = 0
        for train_batch in train_set.get_train_batch(
            max_set_size=options["max_set_size"],
            ...
            (
                ...
                tp=epoch_tp, fp=epoch_fp, fn=epoch_fn
            )
        ...
        if epoch % options["eval_epoch_step"] == 0 and epoch != 0:
            ...
            vocab = dev_set.vocab
            cls_pred = cluster_predict.set_generation(
                model,
                vocab,
                size_opt_clus=options["size_opt_clus"],
                max_K=options["max_K"],
            )
            ...
            tb_writer.add_scalar(
                "val-cluster/em", metrics_cls["num_of_exact_set_prediction"], epoch
            )
            tb_writer.add_scalar(
                "val-cluster/mwm_jaccard",
                metrics_cls["maximum_weighted_match_jaccard"],
                epoch,
            )
            tb_writer.add_scalar(
                "val-cluster/inst_precision", metrics_cls["pair_precision"], epoch
            )
            tb_writer.add_scalar(
                "val-cluster/inst_recall", metrics_cls["pair_recall"], epoch
            )
            tb_writer.add_scalar("val-cluster/inst_f1", metrics_cls["pair_f1"], epoch)
            tb_writer.add_scalar(
                "val-cluster/cluster_num",
                metrics_cls["num_of_predicted_clusters"],
                epoch,
            )
            ...
            my_logger.info(
                "    val/em: {}".format(metrics_cls["num_of_exact_set_prediction"])
            )
            my_logger.info(
                "    val/mwm_jaccard: {}".format(
                    metrics_cls["maximum_weighted_match_jaccard"]
                )
            )
            my_logger.info(
                "    val/inst_precision: {}".format(metrics_cls["pair_precision"])
            )
            my_logger.info("    val/inst_recall: {}".format(metrics_cls["pair_recall"]))
            my_logger.info("    val/inst_f1: {}".format(metrics_cls["pair_f1"]))
            my_logger.info(
                "    val/cluster_num: {}".format(
                    metrics_cls["num_of_predicted_clusters"]
                )
            )
            my_logger.info(
                "    val/clus_size2num_pred_clus: {}".format(
                    metrics_cls["cluster_size2num_of_predicted_clusters"]
                )
            )
        ...
        if epoch - last_step > options["early_stop"]:
            print(
                "Early stop by {} steps, best {}: {}, best step: {}".format(
                    epoch, early_stop_metric_name, best_early_stop_metric, last_step
                )
            )
            break
    ...
    my_logger.info(
        "Loading model: {}/best_steps_{}.pt".format(options["save_dir"], last_step)
    )
    ...
    vocab = dev_set.vocab
    cls_pred = cluster_predict.set_generation(
        model, vocab, size_opt_clus=options["size_opt_clus"], max_K=options["max_K"]
    )
    cls_true = dev_set.positive_sets
    metrics_cls = evaluator.evaluate_clustering(cls_pred, cls_true)
    ...
    predicted_sets = []
    for set_idx in cls_pred:
        wds = [dev_set.index2word[idx] for idx in set_idx]
        predicted_sets.append(wds)
    ...
    results.add("cls_pred", predicted_sets)
...
if __name__ == "__main__":
    ...
    # Add Python Logger
    logger = my_logger(name="exp", log_path=writer.file_writer.get_logdir())  # type: ignore
    logger.setLevel(0)
    ...
    lang_fi = "./data/{}/combined.{}".format(
        options["dataset"], options["pretrained_embedding"]
    )
    visn_fi = "./data/{}/Fileds.{}".format(
        options["dataset"], options["pretrained_embedding"]
    )
    lang_embedding, index2word, word2index, vocab_size, embed_dim = load_embedding(
        lang_fi
    )
    field_embedding, index2word, word2index, vocab_size, embed_dim = load_embedding(
        visn_fi
    )
    logger.info(
    ...
    options["lang_embedding"] = lang_embedding
    options["field_embedding"] = field_embedding
    ...
    train_set_full = element_set.ElementSet(
        "train_set_full", options["data_format"], options, raw_data_string
    )
    ...
    if args.mode == "train":
        ...
        predicted_sets = metrics.metrics["cls_pred"]
        with open(f"results/{args.dataset}_predicted_sets.txt", "w") as f:
    ...
    # Model prediction
        ...
        clusters = cluster_predict.set_generation(
            model, vocab, threshold=0.5, eid2ename=test_set.index2word
        )
