...
def calculate_km_matching_score(weight_nm):
    ...
    lx = [0.0 for i in range(n + 1)]
    ly = [0.0 for i in range(n + 1)]
...
def evaluate_set_instance_prediction(model, dataset):
    ...
    for test_batch in dataset.get_test_batch(
        max_set_size=max_set_size, batch_size=batch_size
    ):
        ...
        setEmbed = model.lang_sspm.node_transform(test_batch["set"]) * mask
        setEmbed = model.lang_sspm.node_pooler(setEmbed, dim=1)
        instEmbed = model.lang_sspm.node_transform(test_batch["inst"]).squeeze_(1)
        setScores = model.lang_sspm.scorer(setEmbed)
        setInstSumScores = model.lang_sspm.scorer(setEmbed + instEmbed)
        lang_score_diff = setInstSumScores - setScores

        lang_prediction = F.sigmoid(lang_score_diff)

        setEmbed = model.field_sspm.node_transform(test_batch["set"]) * mask
        setEmbed = model.field_sspm.node_pooler(setEmbed, dim=1)
        instEmbed = model.field_sspm.node_transform(test_batch["inst"]).squeeze_(1)
        setScores = model.field_sspm.scorer(setEmbed)
        setInstSumScores = model.field_sspm.scorer(setEmbed + instEmbed)
        visn_score_diff = setInstSumScores - setScores

        visn_prediction = F.sigmoid(visn_score_diff)

        if model.device_id != -1:
            lang_prediction = lang_prediction.to(torch.device("cpu"))
            visn_prediction = lang_prediction.to(torch.device("cpu"))

        lang_cur_pred = (lang_prediction > 0.5).squeeze().numpy()
        visn_cur_pred = (visn_prediction > 0.5).squeeze().numpy()

        y_pred += list(lang_cur_pred)

        target = test_batch["label"].float()
        loss = model.criterion(visn_score_diff, lang_score_diff, target).item()
        if model.device_id != -1:
    ...
    for set_size in set_size2correct:
        set_size2accuracy[set_size] = (
            set_size2correct[set_size] / set_size2num[set_size]
        )
...
def evaluate_clustering(cls_pred, cls_true):
    ...
    if len(F_K) == 0:
        ...
        eps = 1e-6
        pair_f1 = (
            2 * pair_precision * pair_recall / (pair_precision + pair_recall + eps)
        )
    ...
    metrics = {
        ...
        "pair_f1": pair_f1,
        "predicted_clusters": cls_pred,
        "num_of_predicted_clusters": num_of_predict_clusters,
