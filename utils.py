...
def check_model_consistency(args):
    ...
    if args.use_pair_feature == 0 and not args.modelName.startswith("np_"):
        return (
            False,
            'model without string pair features must has name starting with "np_"',
        )
    elif args.use_pair_feature == 1 and args.modelName.startswith("np_"):
        return (
            False,
            'model with string pair features cannot has name starting with "np_"',
        )
    elif args.loss_fn == "margin_rank" and not args.modelName.endswith("s"):
        return (
            False,
            "model trained with MarginRankingLoss must have the combiner that output a single "
            "scalar for set-instance pair (i.e., ends with Sigmoid Function)",
        )
    elif args.loss_fn != "margin_rank" and args.modelName.endswith("s"):
        return (
            False,
            "model not trained with MarginRankingLoss cannot have the combiner that output a single "
            "scalar for set-instance pair (i.e., ends with Sigmoid Function)",
        )
    elif args.loss_fn == "self_margin_rank" and "_sd_" not in args.modelName:
        return (
            False,
            "model trained with self MarginRankingLoss must have the combiner that "
            "based on score difference (sd)",
        )
    elif args.loss_fn != "self_margin_rank" and "_sd_" in args.modelName:
        return (
            False,
            "model not trained with self-based MarginRankingLoss cannot have the combiner that "
            "based on score difference (sd)",
        )
    else:
        return True, ""
