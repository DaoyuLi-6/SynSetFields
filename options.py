...
def read_options():
    ...
    parser.add_argument(
        ...
        default="train",
        type=str,
        choices=["train", "eval", "cluster_predict"],
    ...
    parser.add_argument("-fieldSize", default=5, type=int, help="filed size")
    parser.add_argument(
        "-node-hiddenSize",
        default=250,
        type=int,
        help="hidden size used in node post_embedder",
    )
    parser.add_argument(
        "-combine-hiddenSize",
        default=500,
        type=int,
        help="hidden size used in combiner",
    )
    parser.add_argument(
        "-max-set-size", default=50, type=int, help="maximum size for training batch"
    )
    ...
    parser.add_argument(
        ...
        help="loss function used in training model",
    )
    parser.add_argument(
        "-loss-w",
        default=0.7,
    ...
    parser.add_argument(
        "-epochs", default=2000, type=int, help="number of epochs for training"
    )
    parser.add_argument(
        "-neg-sample-size",
        default=20,
    ...
    parser.add_argument(
        "-neg-sample-method",
        default="complete_random",
        type=str,
        choices=["complete_random", "share_token", "mixture"],
        help="negative sampling method",
    )

    # Regularization parameters
    parser.add_argument(
        "-dropout", default=0.3, type=float, help="Dropout between layers"
    )
    parser.add_argument(
        "-early-stop", default=100, type=int, help="early stop epoch number"
    )
    parser.add_argument(
        "-eval-epoch-step",
        default=5,
        type=int,
        help="average number of epochs for evaluation",
    )
    parser.add_argument(
        "-random-seed",
        default=5417,
    ...
    parser.add_argument(
        ...
        default=-1,
        type=int,
        help="maximum cluster number, -1 means auto-infer",
    )
    parser.add_argument(
        "-T", default=1.0, type=int, help="temperature scaling, 1.0 means no scaling"
    )
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="device to use for iterate data, -1 means cpu",
    )

    # Model saving/loading options
    parser.add_argument(
        "-save-dir", default="./snapshots/", type=str, help="location to save models"
    )
    parser.add_argument(
        "-load-model", default="", type=str, help="path to loaded model"
    )
    parser.add_argument(
        "-snapshot", default="", type=str, help="path to model snapshot"
    )
    parser.add_argument(
        "-tune-result-file",
        default="tune_prefix",
    ...
    if args.mode == "train":
        ...
        args.save_dir = os.path.join(
            args.save_dir, current_time + "_" + args.tune_result_file
        )
